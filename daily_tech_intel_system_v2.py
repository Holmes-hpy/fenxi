...
FIFTEEN_FIVE_INDUSTRIES = {
    "人工智能": [
        "人工智能", "AI", "大模型", "生成式AI", "深度学习",
        "机器学习", "计算机视觉", "自然语言处理", "机器人",
        "人形机器人", "智能驾驶", "自动驾驶"
    ],
    "半导体": [
        "半导体", "芯片", "集成电路", "IC", "先进制程",
        "7nm", "14nm", "先进封装", "光刻", "EUV",
        "EDA", "晶圆", "国产替代", "中芯国际"
    ],
    "新能源": [
        "新能源", "光伏", "风电", "储能", "氢能",
    ...
    "数字经济": [
        "数字经济", "数据中心", "算力", "云计算",
        "大数据", "工业互联网", "数字产业化", "产业数字化",
        "东数西算", "数据要素", "数字孪生"
    ],
    "十五五规划": [
        "十五五规划", "第十四个五年规划", "国民经济和社会发展",
        "中长期规划", "高质量发展", "现代化产业体系"
    ],
    "新材料": [
        "新材料", "稀土", "碳纤维", "石墨烯",
        "第三代半导体", "碳化硅", "氮化镓", "SiC", "GaN"
    ],
    "生物医药": [
        "生物医药", "创新药", "基因编辑", "mRNA",
        "医疗器械", "医疗健康", "生物科技"
...
RELIABLE_SOURCES = [
    ...
    "huawei.com", "smic.com", "catl.com", "byd.com"
]
...
REVERSE_SIGNAL_KEYWORDS = {
    ...
    "技术词汇": ["定律", "战法", "秘籍", "绝招", "心法", "神器", "法宝", "密码", "钥匙"],
    "操作词汇": ["抄底", "逃顶", "上车", "下车", "接力", "打板", "追涨", "杀跌", "核按钮"]
}

# 缠论分析知识库
CHAN_THEORY_KNOWLEDGE = {
    "buy_points": {
        "first": {
            "name": "第一类买点",
            "description": "下跌趋势结束，背驰形成的底部",
            "triggers": ["政策底", "技术底", "估值底", "重大利好", "超跌反弹"]
        },
        "second": {
            "name": "第二类买点",
            "description": "第一类买点后的回调确认点",
            "triggers": ["回调企稳", "二次确认", "缩量回踩", "支撑有效"]
        },
        "third": {
            "name": "第三类买点",
            "description": "突破中枢后的回抽确认",
            "triggers": ["趋势确立", "放量突破", "新高确认", "强势上涨"]
        }
    },
    "pivot_levels": {
        "policy": "政策支撑中枢",
...
class DailyTechIntelSystemV2:
    ...
    def search_news_real(self, query, num=5):
        """使用 WebSearch 工具搜索新闻"""
        try:
            print(f"🔍 正在搜索: {query}")
            ...
            try:
                ...
                import inspect
                # 检查是否有可用的 search 工具
                if 'search' in globals() or 'search' in dir(sys.modules.get('__main__', {})):
                    search_results = search(query=query, num=num)
                    return self._process_search_results(search_results, query)
            except Exception as e:
                pass
            
            # 如果 WebSearch 不可用，使用示例数据
            print(f"⚠️ 搜索工具不可用，使用示例数据")
            return self._get_sample_news(query)

        except Exception as e:
            print(f"❌ 搜索失败: {e}")
            return self._get_sample_news(query)

    def _process_search_results(self, search_results, query):
        """处理 WebSearch 返回的结果"""
        processed = []
    ...
    def _get_sample_news(self, query):
        """获取示例新闻数据（备用方案）"""
        samples = [
            {
                "title": "工信部印发人工智能创新发展行动计划（2025-2027年）",
                "url": "https://www.miit.gov.cn/example/ai-plan",
                "snippet": "工信部近日印发《人工智能创新发展行动计划》，提出到2027年我国人工智能核心产业规模突破3万亿元，培育一批具有国际竞争力的AI企业。",
                "search_keyword": query
            },
            {
                "title": "中芯国际宣布14nm工艺良率提升至98%，产能规划超预期",
                "url": "https://www.smic.com/example/process",
                "snippet": "中芯国际最新财报显示，14nm及以下先进制程良率已达到98%，2025年产能计划扩产30%，主要面向5G、AIoT等领域。",
                "search_keyword": query
            },
            {
                "title": "国家能源局：2025年新能源装机目标再上调",
                "url": "https://www.nea.gov.cn/example/energy",
                "snippet": "国家能源局发布最新规划，2025年风电光伏新增装机目标上调至250GW以上，储能配套比例不低于15%。",
                "search_keyword": query
            },
            {
                "title": "震惊！这个板块即将暴涨10倍！内部消息泄露！",
                "url": "https://www.fake-news.com/example/scam",
                "snippet": "刚刚得到内幕消息，主力已经悄悄建仓，赶紧上车！错过这次再等十年！",
                "search_keyword": query
            },
            {
                "title": "发改委：数字经济核心产业占比目标提升至15%",
                "url": "https://www.ndrc.gov.cn/example/digital",
                "snippet": "发改委副主任表示，十五五期间数字经济核心产业增加值占GDP比重目标从10%提升至15%，重点支持算力基础设施建设。",
                "search_keyword": query
            }
        ]
        return samples
    ...
    def crawl_all_news(self):
        ...
        for industry, keywords in FIFTEEN_FIVE_INDUSTRIES.items():
            ...
            for query in search_queries[:2]:  # 每个领域2个搜索query
                results = self.search_news_real(query, num=3)
    ...
    def _identify_subject(self, title, snippet):
        """识别事件主体"""
        subjects = {
            "government": ["国务院", "发改委", "工信部", "科技部", "财政部", "央行", "证监会", "银保监会"],
            "enterprise": ["华为", "中芯国际", "宁德时代", "比亚迪", "腾讯", "阿里", "百度", "美团", "京东"],
            "industry": ["行业协会", "产业联盟", "标准化组织"]
        }
    ...
    def analyze_reverse_signal(self, news):
        ...
        if signal_analysis["signal_score"] >= 3:
            ...
            if signal_analysis["signal_score"] >= 5:
                signal_analysis["analysis"] += "高度疑似标题党或资本雇佣枪手发文，建议反向思考！"
            else:
    ...
    def analyze_chan_theory(self, news, core_info):
        ...
        # 分析第一类买点
        first_buy_triggers = CHAN_THEORY_KNOWLEDGE["buy_points"]["first"]["triggers"]
        ...
        # 分析第二类买点
        second_buy_triggers = CHAN_THEORY_KNOWLEDGE["buy_points"]["second"]["triggers"]
        ...
        # 分析第三类买点
        third_buy_triggers = CHAN_THEORY_KNOWLEDGE["buy_points"]["third"]["triggers"]
        ...
        if core_info["event_type"] == "政策发布":
            chan_analysis["pivot_type"] = CHAN_THEORY_KNOWLEDGE["pivot_levels"]["policy"]
        elif core_info["event_type"] == "技术突破":
            chan_analysis["pivot_type"] = CHAN_THEORY_KNOWLEDGE["pivot_levels"]["tech"]
        else:
            chan_analysis["pivot_type"] = CHAN_THEORY_KNOWLEDGE["pivot_levels"]["industry"]
