...
def update_market_knowledge(data):
    ...
    # 获取大盘指数信息
    sh_index = market_index.get("上证指数", {})
    sz_index = market_index.get("深证成指", {})
    cyb_index = market_index.get("创业板指", {})
...
| 指数 | 收盘点位 | 涨跌幅 | 成交量 |
|------|----------|--------|--------|
| 上证指数 | {sh_index.get('close', 0):.2f} | {sh_index.get('change_pct', 0):+.2f}% | {sh_index.get('volume', 0):.2f}亿 |
| 深证成指 | {sz_index.get('close', 0):.2f} | {sz_index.get('change_pct', 0):+.2f}% | {sz_index.get('volume', 0):.2f}亿 |
| 创业板指 | {cyb_index.get('close', 0):.2f} | {cyb_index.get('change_pct', 0):+.2f}% | {cyb_index.get('volume', 0):.2f}亿 |
...
"""
    ...
    if dragon_tiger and dragon_tiger.get("records"):
        ...
        report += "| 股票名称 | 代码 | 涨跌幅 | 上榜原因 |\n"
        report += "|----------|------|--------|----------|\n"
        for record in dragon_tiger.get("records", [])[:10]:
            name = record.get("name", "")
            code = record.get("code", "")
            change = record.get("change_pct", 0)
            reason = record.get("reason", "")
            report += f"| {name} | {code} | {change:+.2f}% | {reason} |\n"
    else:
        report += "### 5.1 上榜股票汇总\n今日暂无龙虎榜数据。\n"
