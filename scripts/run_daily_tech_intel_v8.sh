#!/bin/bash
# ================================================================================
# 每日科技前沿资讯自动化任务 - Shell执行脚本（v8.0 多源融合增强版）
# ================================================================================
# 执行时间: 每日下午14:30（工作日）
# 功能: 多源融合搜索科技资讯、分析、生成报告、知识沉淀、反向信号识别
...
LOG_FILE="$LOG_DIR/daily_tech_intel_v8_${DATE}_${SESSION}.log"

# 记录开始时间
echo "========================================" >> "$LOG_FILE"
echo "🚀 科技前沿资讯自动化任务【v8.0 多源融合增强版】启动【${SESSION}】" >> "$LOG_FILE"
echo "📅 执行日期: $DATE" >> "$LOG_FILE"
echo "⏰ 执行时间: $(date '+%Y-%m-%d %H:%M:%S')" >> "$LOG_FILE"
echo "📡 搜索源: 百度新闻 + 搜狗新闻 + 财经网站 + RSS订阅" >> "$LOG_FILE"
echo "========================================" >> "$LOG_FILE"

# 切换到项目目录
cd "$PROJECT_DIR"

# 执行Python分析脚本（v8.0 多源融合增强版）
echo "📡 执行资讯分析（v8.0 多源融合增强版）..." >> "$LOG_FILE"
python3 daily_tech_intel_system_v8.py "$SESSION" >> "$LOG_FILE" 2>&1
