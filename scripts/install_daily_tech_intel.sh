...
PLIST_MORNING_SOURCE="$PROJECT_DIR/config/com.user.dailytechintel.morning.plist"
PLIST_AFTERNOON_SOURCE="$PROJECT_DIR/config/com.user.dailytechintel.afternoon.plist"

PLIST_MORNING_TARGET="$HOME/Library/LaunchAgents/com.user.dailytechintel.morning.plist"
PLIST_AFTERNOON_TARGET="$HOME/Library/LaunchAgents/com.user.dailytechintel.afternoon.plist"
...
# 验证任务是否加载成功
echo "🔍 验证任务状态..."
MORNING_LOADED=$(launchctl list | grep dailytechintel.morning)
AFTERNOON_LOADED=$(launchctl list | grep dailytechintel.afternoon)

if [ -n "$MORNING_LOADED" ]; then
    echo "   ✅ 上午任务 (dailytechintel.morning) - 已加载"
else
    echo "   ❌ 上午任务 - 未加载成功"
fi

if [ -n "$AFTERNOON_LOADED" ]; then
    echo "   ✅ 下午任务 (dailytechintel.afternoon) - 已加载"
else
    echo "   ❌ 下午任务 - 未加载成功"
fi
echo ""
...
echo "📋 任务信息:"
echo "   【上午任务】"
echo "   - 任务名称: com.user.dailytechintel.morning"
echo "   - 执行时间: 每个工作日上午 9:30"
echo ""
echo "   【下午任务】"
echo "   - 任务名称: com.user.dailytechintel.afternoon"
...
echo ""
echo "   手动执行上午任务:"
echo "      launchctl start com.user.dailytechintel.morning"
echo ""
echo "   手动执行下午任务:"
echo "      launchctl start com.user.dailytechintel.afternoon"
echo ""
echo "   停止任务:"
echo "      launchctl stop com.user.dailytechintel.morning"
echo "      launchctl stop com.user.dailytechintel.afternoon"
echo ""
echo "   卸载任务:"
