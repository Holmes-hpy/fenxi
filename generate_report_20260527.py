...
def load_data():
    """加载收集的市场数据"""
    data_file = Path(__file__).parent / "reports" / "收盘后数据收集_2026-05-27.json"
...
def update_knowledge_base(data):
    ...
    if market_file.exists():
        with open(market_file, "a", encoding="utf-8") as f:
            f.write(f"\n\n## 2026-05-27 市场观察\n\n")
            f.write("### 今日市场特点\n")
            f.write(f"- 北向资金: 合计{'-' if northbound.get('total', 0) < 0 else '+'}{abs(northbound.get('total', 0)):.2f}亿元\n")
            f.write(f"- 市场情绪: {'偏谨慎' if northbound.get('total', 0) < 0 else '偏乐观'}\n")
            f.write(f"- 领涨行业: {top_industries[0].get('name', '未知') if top_industries else '未知'}\n")
        ...
        with open(industry_file, "a", encoding="utf-8") as f:
            ...
            if top_industries:
                for i, industry in enumerate(top_industries[:5], 1):
                    f.write(f"- 第{i}名: {industry.get('name', '')} {industry.get('change_pct', 0):.2f}%\n")
    
    return True


def check_yesterday_selection():
    """检查昨日选股（贵州茅台）表现"""
    knowledge_dir = Path(__file__).parent / "knowledge"
    mistake_file = knowledge_dir / "mistake_log.md"
    
    # 贵州茅台昨日（2026-05-26）收盘1285.88元，今日收盘1273.38元
    # 预测：震荡偏涨，置信度中
    # 实际：-0.97%
    ...
    if mistake_file.exists():
        with open(mistake_file, "a", encoding="utf-8") as f:
            f.write(f"\n\n## 2026-05-27 选股复盘\n\n")
            f.write("### 昨日选股回顾\n")
            f.write("- **股票**: 贵州茅台(600519)\n")
            f.write("- **昨日收盘价**: 1285.88元\n")
            f.write("- **今日收盘价**: 1273.38元\n")
            f.write("- **涨跌幅**: -0.97%\n")
            f.write("- **预测结果**: 震荡偏涨，置信度中\n")
            f.write("- **实际结果**: 小幅下跌，预测❌错误\n")
            f.write("- **反思**: 市场整体偏弱，北向资金净流出，对白酒板块形成压力\n")
...
def main():
    ...
    # 3. 保存报告
    print("\n3️⃣ 保存报告文件...")
    report_file = Path(__file__).parent / "reports" / "收盘后学习_2026-05-27.md"
