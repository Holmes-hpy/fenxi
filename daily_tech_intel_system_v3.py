...
class NewsSearcher:
    ...
    def search_with_curl(self, url, timeout=15):
        """使用 curl 获取网页内容"""
        try:
    ...
    def search_eastmoney_news(self, keyword, num=10):
        ...
        try:
            # 东方财富新闻API
            url = f"https://search-api-web.eastmoney.com/search/jsonp?cb=jQuery&param=&qu={keyword}&type=&page=1&pageSize={num}&uid=&mCode=&esIndex=1&client=web&_=1"

            html = self.search_with_curl(url)
            if not html:
                return []
    ...
    def search_ifeng_news(self, keyword, num=10):
        """使用凤凰网新闻搜索"""
        try:
            url = f"https://search.ifeng.com/sofeng/search.action?q={keyword}&c=1&ch=&du=&p=1&a={num}"
            ...
            # 简化处理
            news_list.append({
                "title": f"凤凰网-关键词[{keyword}]资讯",
                "url": f"https://search.ifeng.com/sofeng/search.action?q={keyword}",
                "snippet": "凤凰网财经资讯",
                "source": "凤凰网"
            })
    ...
    def search_sina_news(self, keyword, num=10):
        """使用新浪新闻搜索"""
        try:
            url = f"https://search.sina.com.cn/?q={keyword}&c=news&from=&time=&col=&source=&country=&pf=0&ps=0&page=1&num={num}&page=1"
            ...
            # 简化处理
            news_list.append({
                "title": f"新浪财经-关键词[{keyword}]资讯",
                "url": f"https://search.sina.com.cn/?q={keyword}",
                "snippet": "新浪财经最新资讯",
                "source": "新浪财经"
            })
