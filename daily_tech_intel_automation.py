...
使用方式:
  python daily_tech_intel_automation.py
  python daily_tech_intel_automation.py --date 2026-06-24

注意: 此脚本设计为被外部自动化系统调用，WebSearch由外部提供
"""
...
SEARCH_KEYWORDS = {
    ...
    "数字经济": [
        "数字经济 数据中心 算力 2026 site:gov.cn",
        "东数西算 云计算 2026",
        "工业互联网 数字化转型 2026"
    ],
    "十五五规划": [
        "十五五规划 产业政策 2026 site:gov.cn",
        "战略性新兴产业 政策 2026",
        "新质生产力 发展规划 2026"
    ]
}
...
class ReportGenerator:
    ...
    def generate_comprehensive_report(self, analyzed_news: List[Dict[str, Any]]) -> str:
        ...
        report.append(f"**报告版本**: v3.0 (WebSearch增强版)")
...
class DailyIntelAutomation:
    ...
    def load_news_from_file(self) -> List[Dict[str, Any]]:
        """从文件加载搜索结果（由外部系统提供）"""
        news_file = REPORT_DIR / f"{self.target_date}_search_results.json"
        if news_file.exists():
            with open(news_file, 'r', encoding='utf-8') as f:
                return json.load(f)
        return []

    def save_search_results(self, news_list: List[Dict[str, Any]]):
        """保存搜索结果到文件（供外部系统调用）"""
        news_file = REPORT_DIR / f"{self.target_date}_search_results.json"
        with open(news_file, 'w', encoding='utf-8') as f:
            json.dump(news_list, f, ensure_ascii=False, indent=2)
        print(f"📁 搜索结果已保存: {news_file}")

    def get_search_keywords(self) -> Dict[str, List[str]]:
        """获取搜索关键词"""
        return SEARCH_KEYWORDS

    def execute(self, news_list: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        """执行完整分析流程"""
        print("=" * 80)
        ...
        if news_list is None:
            news_list = self.load_news_from_file()

        if not news_list:
            print("   ⚠️ 未找到资讯数据，请先运行搜索任务或提供资讯列表")
            print("   💡 提示: 使用 save_search_results() 方法保存搜索结果")
            return {
                "status": "error",
                "message": "未找到资讯数据"
            }
...
def main():
    """主函数入口"""
    parser = argparse.ArgumentParser(description="科技前沿资讯分析系统 - 自动化版本")
    ...
    # 执行分析
    executor = DailyIntelAutomation(target_date=args.date)

    # 提示用户需要先提供搜索结果
    print("\n" + "=" * 80)
    print("⚠️  注意: 此脚本需要外部提供搜索结果")
    print("=" * 80)
    print("\n使用方式:")
    print("1. 通过外部自动化系统调用 save_search_results() 保存搜索结果")
    print("2. 或直接在代码中传入 news_list 参数执行 execute(news_list)")
    print("\n示例:")
    print("  executor = DailyIntelAutomation()")
    print("  news_list = [{'title': '...', 'snippet': '...', 'url': '...', 'source': '...'}]")
    print("  result = executor.execute(news_list)")
    print("=" * 80 + "\n")
