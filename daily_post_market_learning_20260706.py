...
1. **指数表现**：今日市场整体{'呈现上涨态势' if sum(v.get('change_pct', 0) for v in market_index.values() if isinstance(v, dict)) > 0 else '呈现调整态势'}，{'各指数分化明显' if len(set(v.get('change_pct', 0) for v in market_index.values() if isinstance(v, dict))) > 2 else '各指数走势较为一致'}。
