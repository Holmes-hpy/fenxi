...
def collect_market_data():
    ...
    try:
        ...
        print(f"   ✅ 完成: {data['northbound_flow']}")
    except Exception as e:
        print(f"   ❌ 失败: {e}")
    ...
    try:
        ...
        if hot_stocks_df is not None and not hot_stocks_df.empty:
            ...
            print(f"   ✅ 完成: 获取到 {len(data['hot_stocks'])} 只热点股票")
        else:
            print("   ⚠️ 暂无热点股票数据")
    except Exception as e:
        print(f"   ❌ 失败: {e}")
...
| 指数 | 涨跌幅 | 状态 |
|------|--------|------|
| 上证指数 | {market_index.get('shanghai', {}).get('change_pct', 0):+.2f}% | {'上涨' if market_index.get('shanghai', {}).get('change_pct', 0) > 0 else '下跌'} |
| 深证成指 | {market_index.get('shenzhen', {}).get('change_pct', 0):+.2f}% | {'上涨' if market_index.get('shenzhen', {}).get('change_pct', 0) > 0 else '下跌'} |
| 创业板指 | {market_index.get('gem', {}).get('change_pct', 0):+.2f}% | {'上涨' if market_index.get('gem', {}).get('change_pct', 0) > 0 else '下跌'} |

### 北向资金动向

- **沪股通**: {northbound.get('hgt', 0):.2f} 亿元 {'📈 净流入' if northbound.get('hgt', 0) > 0 else '📉 净流出'}
- **深股通**: {northbound.get('sgt', 0):.2f} 亿元 {'📈 净流入' if northbound.get('sgt', 0) > 0 else '📉 净流出'}
- **合计**: {northbound.get('total', 0):.2f} 亿元 {'📈 净流入' if northbound.get('total', 0) > 0 else '📉 净流出'}

### 今日市场特点

1. **市场情绪**: {'偏乐观' if northbound.get('total', 0) > 0 else '偏谨慎'}
2. **热点板块**: {', '.join([i.get('name', '') for i in top_industries[:3]]) if top_industries else '暂无数据'}
3. **弱势板块**: {', '.join([i.get('name', '') for i in bottom_industries[:3]]) if bottom_industries else '暂无数据'}
"""
...
"""
    ...
    theme_desc = f"{themes[0][0] if themes else '暂无'}" if themes else "暂无"
    
    report += f"""
### 4.2 题材特点分析
今日市场热点主要集中在：
1. **{themes[0][0] if themes else '暂无数据'}** - 出现 {themes[0][1] if themes else 0} 次
2. **{themes[1][0] if len(themes) > 1 else '暂无数据'}** - 出现 {themes[1][1] if len(themes) > 1 else 0} 次
3. **{themes[2][0] if len(themes) > 2 else '暂无数据'}** - 出现 {themes[2][1] if len(themes) > 2 else 0} 次
...
2. **板块轮动**: 今日领涨板块为{top_industries[0].get('name', '暂无数据') if top_industries else '暂无数据'}，领跌板块为{bottom_industries[-1].get('name', '暂无数据') if bottom_industries else '暂无数据'}。
3. **热点题材**: 今日最热题材为{theme_desc}，市场情绪活跃。

### 6.2 市场情绪研判
- **整体情绪**: {'📈 偏乐观' if northbound.get('total', 0) > 0 else '📉 偏谨慎'}
- **热点持续性**: {top_industries[0].get('name', '暂无数据') if top_industries else '暂无数据'}板块表现强势，需观察持续性。
...
### 7.2 明日操作建议
1. **关注热点持续性**: 继续关注{top_industries[0].get('name', '科技板块') if top_industries else '科技板块'}的持续性。
...
- **行业动态**: {top_industries[0].get('name', '科技板块') if top_industries else '科技板块'}的强势表现
- **题材分析**: 热点题材的扩散规律
...
### 9.2 明日学习计划
1. 继续观察{top_industries[0].get('name', '科技板块') if top_industries else '科技板块'}的持续性
2. 分析北向资金流出的原因
3. 关注政策面消息
4. 更新选股池
